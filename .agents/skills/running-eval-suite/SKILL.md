...
```
.agents/skills/running-eval-suite/models/<model>/config.yaml
```
...
| **B — job** | `cd /sgl-workspace/sglang-omni && python .agents/skills/running-eval-suite/runner.py run ... >> <run-dir>/run.log 2>&1` |
...
2. Run precheck. The runner creates `<repo>/.eval-runs/<utc-ts>/` on
   ...
   invocation:
   ```
   python .agents/skills/running-eval-suite/runner.py \
...
```
.agents/skills/running-eval-suite/
├── SKILL.md
...
3. Verify the locator matches the intended H200 row (or doesn't yet
   ...
   ```bash
   python - <<'EOF'
   import sys; sys.path.insert(0, '.agents/skills/running-eval-suite')
   import runner, yaml; from pathlib import Path
   cfg = yaml.safe_load(open('.agents/skills/running-eval-suite/models/qwen3-omni/config.yaml'))
   for row in cfg['rows']:
