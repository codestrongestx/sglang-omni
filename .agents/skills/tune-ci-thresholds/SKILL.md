...
**Agent runbook:** `.agents/skills/tune-ci-thresholds/AGENT-PRECHECK.md` —
...
|-----|------|-----------------------------------|
| **A — supervision** | pytest + router/server verbose | `bash .agents/skills/tune-ci-thresholds/tail_calibration_pytest.sh <run-dir>` — resolves the **active** pytest via running process (`--basetemp`), switches immediately when tune.py starts the next test. **Forbidden:** `tail -f $(ls -t …/run*.log \| head -1)` (sticks on completed logs like failed mmmu run1). |
| **B — job** | tune.py milestone lines (stdout) | `cd /sgl-workspace/sglang-omni && python .agents/skills/tune-ci-thresholds/tune.py --model <M> run ... --output-dir <run-dir>` — **no** `tee`, **no** `>>` |
...
```
python .agents/skills/tune-ci-thresholds/tune.py models-list
```
...
python .agents/skills/tune-ci-thresholds/tune.py --model qwen3-omni-v1 run \
...
python .agents/skills/tune-ci-thresholds/tune.py --model tts run \
...
| Mistake | Symptom | Fix |
|---------|---------|-----|
| Tab A tails `<run-dir>/run.log` during `tune.py run` | Tab A and Tab B show **identical** milestone lines; no pytest/router output | Kill Tab A; run `bash .agents/skills/tune-ci-thresholds/tail_calibration_pytest.sh <run-dir>` |
...
python .agents/skills/tune-ci-thresholds/tune.py --model qwen3-omni-v1 precheck   # or qwen3-asr-v1 / tts
```
...
`.agents/skills/tune-ci-thresholds/AGENT-PRECHECK.md` (full environment +
...
1. Run `python .agents/skills/tune-ci-thresholds/tune.py models-list` to
...
3. Run `python tune.py --model <M> precheck --output-dir <run-dir>`.
   ...
   **`<run-dir>` must live under `.tune-runs/<timestamp>_<label>/`** at
   ...
   `.agents/skills/` or anywhere else under version control — run
...
5. **Before** launching run, **spawn two IDE terminal tabs** per **Two-terminal
   ...
   **Tab A — supervision (pytest + server log; NOT run.log):**
   ```bash
   bash .agents/skills/tune-ci-thresholds/tail_calibration_pytest.sh <run-dir>
   ```

   **Tab B — job (tune.py milestones on stdout — no redirect):**
   ```bash
   cd <repo_root from host profile> && python .agents/skills/tune-ci-thresholds/tune.py --model <M> run ... \
     --output-dir <run-dir>
   ```
   Example (`sglang-h20-ci`): `cd /data/chenyang/sglang-omni && TUNE_VENV_PYTHON=/data/chenyang/.python/omni/bin/python python .agents/skills/tune-ci-thresholds/tune.py ...`
...
## Files in this skill
```
.agents/skills/tune-ci-thresholds/
├── SKILL.md
